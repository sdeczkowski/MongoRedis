...
import { addToDo, getAllToDo, updateToDo, deleteToDo } from "../utils/HandleApiRedis";
...
function Todo() {
  ...
  return (
    <Container>
      <Row>
        <h1 className="text-light text-center mt-2">TodoList Redis</h1>
      ...
      <Row className="d-flex flex-column align-content-center" style={{ overflow: "auto" }}>
        {toDo.map((item) => (
          <ToDo
            key={item.id}
            text={item.data.text}
            updateMode={() => updateMode(item.id, item.data.text)}
